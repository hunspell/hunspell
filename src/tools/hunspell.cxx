...
bool print_filename; // for listing file names in pipe interface
...
void report_bad_color(char const* start, char const* end) {
  fprintf(stderr, gettext("error - invalid color specifier, '%.*s'\n"),
          static_cast<int>(end - start), start);
}

void read_color(char const* start, char const* end) {
  if ((end - start) < 4) {
    report_bad_color(start, end);
    return;
  }

  int which = -1;
  if (strncmp(start, "fn=", 3) == 0)
    which = 0;
  else if (strncmp(start, "ln=", 3) == 0)
    which = 1;
  else if (strncmp(start, "se=", 3) == 0)
    which = 2;
  else if (strncmp(start, "ms=", 3) == 0)
    which = 3;

  char const* c = start + 3;
  bool okay = (which >= 0);
  while (c < end && okay) {
    if (c[1] == ';' || (c + 1) >= end) {
      if (!is_octal(c[0]))
      ...
      okay = false;
    } else if (c[0] != '0' && c[0] != '3' && c[0] != '4') {
      okay = false;
    } else if (!is_octal(c[1])) {
      okay = false;
    } else {
      c += 3;
    }
  ...
  if (okay)
    grep_colors[which] = std::string(start + 3, end);
  else
    report_bad_color(start, end);
}

bool read_colors(char const* envname) {
  char const* colors = getenv(envname);
  if (!colors || !*colors)
    return false;

  while (char const* delim = strchr(colors, ':')) {
    read_color(colors, delim);
    colors = delim + 1;
  }
  if (*colors)
    read_color(colors, colors + strlen(colors));
...
void pipe_interface(Hunspell** pMS, int format, FILE* fileid, char* filename) {
  ...
  std::string filename_prefix = (print_filename) ? currentfilename + std::string(": ") : "";
  char const* maybe_filename = (print_filename ? currentfilename : nullptr);
...
int main(int argc, char** argv) {
  ...
  print_filename = print_filename || ((arg_files > 0) && (argc - arg_files > 1));
  ...
  */
  ...
  if (arg_files == -1) {
    currentfilename = gettext("(standard input)");
